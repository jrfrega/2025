...
Olhando o melhor modelo indicado pelo AIC, 
...
y = predict(mod1, data.frame(advert = adv, price = mean(andy$price)))
lines(adv, y, col = "red", lwd = 2)
y = predict(mod2.1, data.frame(advert = adv, price = mean(andy$price)))
lines(adv, y, col = "blue", lwd = 2)
```

```{r}
plot(andy$price, andy$sales, ylim = c(min(andy$sales), max(andy$sales)), pch = 20, col = "darkgray")
prc = seq(min(andy$price), max(andy$price), length.out = 100)
y = predict(mod1, data.frame(advert = mean(andy$advert), price = prc))
lines(prc, y, col = "red", lwd = 2)
y = predict(mod2.1, data.frame(advert = mean(andy$advert), price = prc))
lines(prc, y, col = "blue", lwd = 2)
plot(mod2.1$residuals); abline(h = 0, col = "darkgray")
plot(mod2.1$residuals, type = "l"); abline(h = 0, col = "darkgray")
plot(mod2$residuals); abline(h = 0, col = "darkgray")
plot(mod2$residuals, type = "l"); abline(h = 0, col = "darkgray")
mhist(mod2.1$residuals %>% scale, breaks = 6)
```
